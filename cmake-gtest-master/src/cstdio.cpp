...
int main() {
    ...
    if (file == nullptr) {
        ...
        fputs("Could not open file\n", stderr);                 // do outputu formatowanego, roznica do fwrite, tutaj argument traktowany jako string wiec zapisze do "\0"

        //fputs("aacacac", stdout);         // to samo tylko ten nie dodaje znaku konca
        //puts("aasdad");                   // a ten dodaje

        exit(1);
    }

    // OUTPUT NIEFORMATOWANY fwrite
    const int SIZE = 10;
    int data[SIZE];
    ...
    int written = fwrite(data, sizeof(int), SIZE, file);            //dane do zapisania, rozmiar pojedynczego elementu, rozmiar danych, gdzie zapisac
    ...
    long pos = ftell(file);                                      // zwraca wskaznik na koniec informacji w pliku file
    // %ld - long int
    printf("Current position in file: %ld\n", pos);

    // Move 10 bytes before the end of file
    fseek(file, -10, SEEK_END);                                 // gdzie szukac, o ile sie poruszac, co druga liczba moze oznaczac
    printf("Current position in file: %ld\n", ftell(file));
