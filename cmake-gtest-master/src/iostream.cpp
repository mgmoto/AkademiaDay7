...
int main() {
    ...
    file.put('A').put('L').put('A');                            // wstawia pojedynczy znak
    ...
    file.close();                                               // jezeli nie zamkniemy pliku, to destruktor i tak zamknie, wiec no big deal
...
std::ifstream in; // dodac otwarcie pliku
std::ofstream out;
...
while(!in.eof())
{
    ...
    int charBufCount = in.gcount();
    for(int i = 0; i<charBuffCount ; ++i)
    {
        out.write(&buf,charBuffCount);
    }
}
